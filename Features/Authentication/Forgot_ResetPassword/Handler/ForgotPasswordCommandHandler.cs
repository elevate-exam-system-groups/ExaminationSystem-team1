...
namespace ExaminationSystem.Features.Authentication.Forgot_ResetPassword.Handler
{
    public class ForgotPasswordHandler : IRequestHandler<ForgotPasswordCommand, RequestResult<ForgotPasswordResponse>>
    {
            ...
            ForgotPasswordCommand request, CancellationToken cancellationToken)
        {
            ...
            var tokenHash = TokenHasher.HashToken(resetToken);      // private method

            var passwordResetToken = new PasswordResetToken
            {
                UserId = user.Id,
                TokenHash = tokenHash,
                ExpiryAt = DateTime.UtcNow.AddMinutes(15), // 15 دقيقة صلاحية
                IsUsed = false,
            ...
            if (!emailSent)
            {
                return RequestResult<ForgotPasswordResponse>.Failure(ErrorCode.EmailSendFailed);
            }
