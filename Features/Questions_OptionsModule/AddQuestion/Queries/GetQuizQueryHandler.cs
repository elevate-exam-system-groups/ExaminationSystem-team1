namespace ExaminationSystem.Features.Questions_OptionsModule.AddQuestion.Queries
{
    ...
    : IRequestHandler<GetQuizQuery, RequestResult<QuizDto>>
    {

        private readonly IGeneralRepository<Quiz> _quizRepo;
        public GetQuizQueryHandler(IGeneralRepository<Quiz> quizRepo)
            => _quizRepo = quizRepo;

        public async Task<RequestResult<QuizDto>> Handle(
             GetQuizQuery request, CancellationToken ct)
        {
